...
void AEnemy::Tick(float DeltaTime)
{
	...
	if (bOverlappingCombatSphere && Alive())
	{
		...
		if (MainCharEnemyRef->CombatTarget == nullptr || MainCharEnemyRef->CombatTarget && MainCharEnemyRef->
			CombatTarget->Health <= 0.f)
		{
...
void AEnemy::MoveToTarget(AMainChar* Target)
{
	if (Alive())
	{
		...
		// Draw debug for nav path points : 
...
void AEnemy::Attack()
{
	if (Alive() && bHasValidTarget)
	{
		...
		if (!bAttacking)
		{
			...
			if (AnimInstance)
			{
				const int Section = FMath::RandRange(0, 3);
				switch (Section)
				{
				case 0:
...
void AEnemy::Disappear()
{
	if (DisappearParticles)
	{
		...
		UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), DisappearParticles,
		                                         DisappearParticleSocket->GetSocketLocation(GetMesh()), FRotator(0.f),
		                                         true);
	}
	Destroy();
}
